...
// Récupération des informations de l'admin connecté
...
<head>
    ...
    <style>
        ...
        .role-id-1 { background: #dc3545; color: white; } /* Admin */
        .role-id-2 { background: #ffc107; color: #212529; } /* Modérateur */
        .role-id-3 { background: #28a745; color: white; } /* Gamer */
        .role-id-4 { background: #6c757d; color: white; } /* Banni */
        .pagination {
            display: flex;
            justify-content: center;
            gap: 10px;
...
<body>
    <div class="admin-container">
        ...
        <div class="admin-header">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <button id="themeToggle" class="btn btn-secondary" style="padding: 8px 12px;">
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; gap: 5px;">
                                    <button class="btn btn-sm btn-primary" onclick="changeRole(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars($user['username']); ?>')">
                                        <i class="fas fa-edit"></i> Changer rôle
                                    </button>
    ...
    <div id="roleModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: var(--white); padding: 30px; border-radius: var(--border-radius); min-width: 400px;">
            <h3>Changer le rôle de <span id="modalUsername"></span></h3>
            <form id="changeRoleForm">
                <input type="hidden" id="modalUserId" name="user_id">
                <div class="form-group">
                    <label for="newRole">Nouveau rôle</label>
                    <select id="newRole" name="role_id" required>
                        <?php foreach ($roles as $role): ?>
                        <option value="<?php echo $role['id']; ?>">
                            <?php echo htmlspecialchars($role['name']); ?>
                        </option>
    ...
    <script>
        ...
        document.getElementById('changeRoleForm').addEventListener('submit', function(e) {
            ...
            fetch('api/admin_change_role.php', {
                method: 'POST',
                body: formData
            })
            .then(res => res.json())
    ...
    <script>
        // Gestion du thème pour la page admin
